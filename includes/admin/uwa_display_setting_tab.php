 ...
 if(isset($_POST['uwa-settings-submit']) == 'Save Changes')
	{	
		if(isset($_POST['uwa_badge_image_url'])) {
			update_option('uwa_badge_image_url', $_POST['uwa_badge_image_url']);
		}
		...
		if(isset($_POST['uwa_schedule_enabled']) && $_POST['uwa_schedule_enabled'] == 1){ 
		...
		if(isset($_POST['uwa_display_reverse_text']) && $_POST['uwa_display_reverse_text'] ==1){
			update_option('uwa_display_reverse_text', "yes");
		} else{
			update_option('uwa_display_reverse_text', "no");
		}

		if(isset($_POST['uwa_display_proxy_text']) && $_POST['uwa_display_proxy_text'] ==1){
            update_option('uwa_display_proxy_text', "yes");
        } else{
            update_option('uwa_display_proxy_text', "no");
        }	
        
		if(isset($_POST['uwa_reverse_text'])){
			update_option('uwa_reverse_text', $_POST['uwa_reverse_text']);
		}
		if(isset($_POST['uwa_proxy_text'])){
			update_option('uwa_proxy_text', $_POST['uwa_proxy_text']);
		}
		
		if(isset($_POST['uwa_show_timer_on_shoppage']) && $_POST['uwa_show_timer_on_shoppage'] ==1){
            update_option('uwa_show_timer_on_shoppage', "yes");
        } else{
            update_option('uwa_show_timer_on_shoppage', "no");
        }
		if(isset($_POST['uwa_enable_product_condition_field'])){	
			update_option('uwa_enable_product_condition_field', "yes");
		} else{
			update_option('uwa_enable_product_condition_field', "no");
		}	
		...
		if(isset($_POST['uwa_enable_timer_field'])){	
			update_option('uwa_enable_timer_field', "yes");
		} else{
			update_option('uwa_enable_timer_field', "no");
		}	
		
		if(isset($_POST['uwa_enable_ending_on_field'])){	
			update_option('uwa_enable_ending_on_field', "yes");
		} else{
			update_option('uwa_enable_ending_on_field', "no");
		}	
		if(isset($_POST['uwa_enable_start_on_field'])){	
			update_option('uwa_enable_start_on_field', "yes");
		} else{
			update_option('uwa_enable_start_on_field', "no");
		}	
		
		if(isset($_POST['uwa_enable_timezone_field'])){	
			update_option('uwa_enable_timezone_field', "yes");
		} else{
			update_option('uwa_enable_timezone_field', "no");
		}	
		
		if(isset($_POST['uwa_enable_reserve_field'])){	
			update_option('uwa_enable_reserve_field', "yes");
		} else{
			update_option('uwa_enable_reserve_field', "no");
		}	
		if(isset($_POST['uwa_show_reserve_price'])){	
			update_option('uwa_show_reserve_price', "yes");
		} else{
			update_option('uwa_show_reserve_price', "no");
		}	
		
		if(isset($_POST['uwa_show_direct_bid']) && $_POST['uwa_show_direct_bid'] ==1){
        ...
        if(isset($_POST['uwa_show_custom_bid']) && $_POST['uwa_show_custom_bid'] ==1){
		...
		if(isset($_POST['uwa_label_custom_bid']) && $_POST['uwa_label_custom_bid'] !=  ""){
 		...
 		if(isset($_POST['uwa_display_wining_losing_text'])){
			$is_updated = update_option('uwa_display_wining_losing_text', $_POST['uwa_display_wining_losing_text']);
			
		}
		if(isset($_POST['uwa_display_wining_text'])){
			update_option('uwa_display_wining_text', $_POST['uwa_display_wining_text']);
		}
		if(isset($_POST['uwa_display_losing_text'])){
			update_option('uwa_display_losing_text', $_POST['uwa_display_losing_text']);
		}


		if(isset($_POST['uwa_winner_live_shop']) && $_POST['uwa_winner_live_shop'] == 1){
            update_option('uwa_winner_live_shop', "yes");
        } else{
            update_option('uwa_winner_live_shop', "no");
        }

        if(isset($_POST['uwa_winner_live_product']) && $_POST['uwa_winner_live_product'] == 1){
            update_option('uwa_winner_live_product', "yes");
        } else{
            update_option('uwa_winner_live_product', "no");
        }

        if(isset($_POST['uwa_winner_live_widget']) && $_POST['uwa_winner_live_widget'] == 1){
            update_option('uwa_winner_live_widget', "yes");
        } else{
            update_option('uwa_winner_live_widget', "no");
        }

        if(isset($_POST['uwa_winner_expired_shop']) && $_POST['uwa_winner_expired_shop'] == 1){
            update_option('uwa_winner_expired_shop', "yes");
        } else{
            update_option('uwa_winner_expired_shop', "no");
        }

        if(isset($_POST['uwa_winner_expired_product']) && $_POST['uwa_winner_expired_product'] == 1){
            update_option('uwa_winner_expired_product', "yes");
        } else{
            update_option('uwa_winner_expired_product', "no");
        }

        if(isset($_POST['uwa_winner_expired_widget']) && $_POST['uwa_winner_expired_widget'] == 1){
	...
	$reverse_text_enable = get_option('uwa_display_reverse_text');
	$reverse_text_checked_enable="";
	if($reverse_text_enable =="yes"){
		$reverse_text_checked_enable = "checked";
	}
	$proxy_text_enable = get_option('uwa_display_proxy_text');
	$proxy_text_checked_enable="";
	if($proxy_text_enable =="yes"){
		$proxy_text_checked_enable = "checked";
	}
	...
	$uwa_enable_product_condition_field_enable="";
		if($uwa_enable_product_condition_field =="yes"){
			$uwa_enable_product_condition_field_enable = "checked";
		}	
	...
	$uwa_enable_timer_field = get_option('uwa_enable_timer_field',"yes");
	$uwa_enable_timer_field_enable="";
		if($uwa_enable_timer_field =="yes"){
			$uwa_enable_timer_field_enable = "checked";
		}
		
	$uwa_enable_ending_on_field = get_option('uwa_enable_ending_on_field',"yes");
	$uwa_enable_ending_on_field_enable="";
		if($uwa_enable_ending_on_field =="yes"){
			$uwa_enable_ending_on_field_enable = "checked";
		}
	$uwa_enable_start_on_field = get_option('uwa_enable_start_on_field',"yes");
	$uwa_enable_start_on_field_enable="";
		if($uwa_enable_start_on_field =="yes"){
			$uwa_enable_start_on_field_enable = "checked";
		}	
		

	$uwa_enable_timezone_field = get_option('uwa_enable_timezone_field',"yes");;
	$uwa_enable_timezone_field_enable="";
		if($uwa_enable_timezone_field =="yes"){
			$uwa_enable_timezone_field_enable = "checked";
		}	
	$uwa_enable_reserve_field = get_option('uwa_enable_reserve_field',"yes");
	$uwa_enable_reserve_field_enable="";
		if($uwa_enable_reserve_field =="yes"){
			$uwa_enable_reserve_field_enable = "checked";
		}	
		...
		<form  method='post' class='uwa_auction_setting_style'>
			<table class="form-table">
				<tbody>							
					...
					<tr>				
						<th scope="row">
							<label for="uwa_show_timer_on_shoppage"><?php _e( 'Show On Shop Page', 'woo_ua' ); ?></label>
						</th>
						<td>
							<input <?php echo $uwa_show_timer_on_shoppage_checked_enable; ?> value="1" name="uwa_show_timer_on_shoppage" type="checkbox">
							<?php _e('Enable/Disable countdown on Shop(Product loop) Page.','woo_ua');?>	
					    </td>
					</tr>
					...
					<tr>				
						<th scope="row">
							<label for="uwa_countdown_format"><?php _e( 'Timer Setting', 'woo_ua' ); ?></label>
						</th>
					...
					<tr>
						<th scope="row"></th>
						<td>
							<input type="checkbox" <?php echo $uwa_enable_timer_field_enable;?> name="uwa_enable_timer_field"  id="uwa_enable_timer_field" value="1"><?php _e('Enable/Disable Timer. This setting lets you Enable/Disable Timer.', 'woo_ua');  ?>
						</td>
					</tr>
					...
					<tr>
					<th>
						<label for="uwa_enable_product_condition_field"><?php _e( 'Enable/Disable Specific fields:', 'woo_ua' ); ?></label>
					</th>
					<td class="uwaforminp">									
						<input type="checkbox" <?php echo $uwa_enable_product_condition_field_enable;?> name="uwa_enable_product_condition_field"  id="uwa_enable_product_condition_field" value="1"><?php _e('Product(item) Condition field.', 'woo_ua');  ?>
				...
				<tr>
					<th scope="row"></th>
					<td>
						<input type="checkbox" <?php echo $uwa_enable_ending_on_field_enable;?> name="uwa_enable_ending_on_field"  id="uwa_enable_ending_on_field" value="1"><?php _e('Ending On date For Live Auction and Future Auction.', 'woo_ua');  ?>
					</td>
				</tr>
				...
				<tr>
					<th scope="row"></th>
					<td>
						<input type="checkbox" <?php echo $uwa_enable_start_on_field_enable;?> name="uwa_enable_start_on_field"  id="uwa_enable_start_on_field" value="1"><?php _e('Start On date For Future Auction.', 'woo_ua');  ?>
					</td>
				</tr>
				...
				<tr>
					<th scope="row"></th>
					<td>
						<input type="checkbox" <?php echo $uwa_enable_timezone_field_enable;?> name="uwa_enable_timezone_field"  id="uwa_enable_timezone_field" value="1"><?php _e('Timezone.', 'woo_ua');  ?>
					</td>
				</tr>
				...
				<tr>
					<th scope="row"></th>
					<td>
						<input type="checkbox" <?php echo $uwa_enable_reserve_field_enable;?> name="uwa_enable_reserve_field"  id="uwa_enable_reserve_field" value="1"><?php _e('Reserve Price Text.', 'woo_ua');  ?>
					</td>
				</tr>
				...
				<tr>
						<th scope="row"></th>
						<td>
						<input <?php echo $proxy_text_checked_enable; ?> value="1" name="uwa_display_proxy_text"
						id="uwa_display_proxy_text" type="checkbox"><?php _e( 'Proxy Auction Text.', 'woo_ua' ); ?>
					   </td>
					</tr>
				...
				<tr>					
						<th scope="row"></th>
						<td>
						<input <?php echo $reverse_text_checked_enable; ?> value="1" name="uwa_display_reverse_text" type="checkbox"><?php _e( 'Reverse Auction Text.', 'woo_ua' ); ?>						
						</td>
					</tr>
					<tr>
						<th scope="row"></th>
				...
				<tr>
					...
					<td class="uwaforminp">									
						<input type="checkbox" <?php echo $uwa_show_reserve_price_field_enable;?> name="uwa_show_reserve_price"  id="uwa_show_reserve_price" value="1"><?php _e('Reserve Price.', 'woo_ua');  ?>
				...
				<!--end NEW VERSION FROM 2.0.1-->
					...
					<tr>
						...
						<td>
							...
							"uwa_watchlists_future_tab" type="checkbox"><?php _e( 'Enable watchlists for future auctions', 'woo_ua' ); ?>
						</td>
					</tr>	
