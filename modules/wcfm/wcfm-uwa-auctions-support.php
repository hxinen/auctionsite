				...
				<p class="uwa_variable_bid_increment_main">		
						...
						<?php 
							...
							<span id="uwa_custom_field_0" class="uwa_custom_field_main">
								<input type="number" min="1" class="uwa_auction_price_fields start_valid" id="start_val_0" data-startid="0" name="uwa_var_inc_val[0][start]" value="" placeholder="<?php _e('Start Price', 'woo_ua'); ?>"/><input type="number" min="1" class="uwa_auction_price_fields end_valid" id="end_val_0" data-endid="0"  name="uwa_var_inc_val[0][end]" value="" placeholder="<?php _e('End Price', 'woo_ua'); ?>"/><input type="number" min="1" class="uwa_auction_price_fields" id="inc_val_0" name="uwa_var_inc_val[0][inc_val]" value="" placeholder="<?php _e('Increment', 'woo_ua'); ?>"/>
							</span>
							<?php } ?>
	...
	if (current_user_can('administrator') or current_user_can('shop_manager')) {		 
		...
		if(!empty($auction_products_array)) {
			...
			foreach($auction_products_array as $auction_product) {			
				...
				$action_view_link .= '<a class="wcfm-action-icon" target="_blank" href="'.get_wcfm_view_auction_url( $auction_product->ID,$product_data ).'""><span class="wcfmfa fa-info text_tip" data-tip="View Detail" data-hasqtip="'.$auction_product->ID.'" aria-describedby="qtip-'.$auction_product->ID.'"></span></a><br>';
